...
public class CartController {
    ...
    public static class GetItemsCart extends HttpServlet {
        ...
        protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
            ...
            String user_id = (String) req.getSession().getAttribute("login");
            ArrayList<MyObject> carts = DB.getData(sql, new String[] { user_id }, new String[] { "id", "user_id",
                    "event_id", "quantity", "note", "price", "event_title", "tickets" });
    ...
    public static class ChangeCartQuantity extends HttpServlet {
        ...
        protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
            ...
            ArrayList<MyObject> result = DB.getData(sql, new String[] { cart_id, cart_id, cart_id },
                    new String[] { "info_type", "info_value" });
            boolean check = false;
    ...
    public static class Payment extends HttpServlet {
        ...
        protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
            ...
            String user_id = (String) req.getSession().getAttribute("login");
            ResourceBundle language = (ResourceBundle) req.getAttribute("language");
            ArrayList<MyObject> carts = DB.getData(
                    "select carts.id, carts.user_id, event_id, quantity, bill_id, note,events.image, events.price as price, events.title as event_title, events.tickets as tickets, transfer_content, sum(quantity * events.price) as amount\n"
                            +
                            "from carts\n" +
                            ...
                            "  and bills.id = ?\n" +
                            "  and carts.user_id = ?\n" +
                            "group by carts.id, carts.user_id, event_id, quantity, bill_id, note, events.image, events.price, events.title, events.tickets, transfer_content",
                    new String[] { bill_id, user_id }, new String[] { "id", "user_id", "event_id", "quantity", "note",
                            "image", "price", "event_title", "tickets", "amount", "transfer_content" });
            ...
            if (carts.size() == 0) {
                ...
                String[] fields = new String[] { "id", "title", "description", "start_date", "end_date", "location",
                        "is_verified", "user_id", "category_id", "tickets", "price", "image", "category_name",
                        "username", "interested", "count" };
                ...
                for (int i = 0; i < carts.size(); i++) {
                    for (int j = 0; j < events.size(); j++) {
                        ...
                        if (carts.get(i).event_id.equals(events.get(j).id) && quantity + bought > tickets) {
                            change = true;
                            if_change += language.getString("quantity_changed").replace("xx", events.get(j).title)
                                    .replace("yy", carts.get(i).quantity)
                                    .replace("zz", String.valueOf(tickets - bought)) + "\n";
                            carts.get(i).quantity = String.valueOf(tickets - bought);
                            sql = "update carts set quantity = " + carts.get(i).quantity + " where id = "
                                    + carts.get(i).id + ";";
                ...
                DB.executeUpdate("update bills set amount = ? where id = ?",
                        new String[] { String.valueOf(amount), bill_id });
